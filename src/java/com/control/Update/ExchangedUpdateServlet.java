 ...
 * @author 钟镇斌
 */
@WebServlet(name = "ExchangedUpdateServlet", urlPatterns = {"/ExchangedUpdateServlet"})
public class ExchangedUpdateServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            flag=operate.update_exchanged(kefang, guke, exchanged_id, custom_name, custom_tel, before_room, before_room_type, before_room_price, after_room, after_room_type, after_room_price, exchanged_time, run_staff_name, run_staff_tel);
             if (flag == 1) {
                out.println("<script language='javascript'>window.location.href='staff.jsp';alert('修改换房成功！')</script>");
            } else {
                out.println("<script language='javascript'>window.location.href='staff.jsp';alert('修改换房失败！')</script>");
            }
        }
    }
