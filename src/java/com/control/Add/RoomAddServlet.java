 ...
 * @author 钟镇斌
 */
@WebServlet(name = "RoomAddServlet", urlPatterns = {"/RoomAddServlet"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
          ...
          String state=request.getParameter("state");
            ...
            if (flag == 1) {
                out.println("<script language='javascript'>window.location.href='staff.jsp';alert('添加客房成功！')</script>");
            } else {
                out.println("<script language='javascript'>window.location.href='staff.jsp';alert('添加客房失败！')</script>");
            }
        }
    }
