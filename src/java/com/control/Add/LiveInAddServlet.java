 ...
 * @author 钟镇斌
 */
@WebServlet(name = "LiveInAddServlet", urlPatterns = {"/LiveInAddServlet"})
public class LiveInAddServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
          ...
          if (flag == 1) {
                out.println("<script language='javascript'>window.location.href='staff.jsp';alert('添加入住成功！')</script>");
            } else {
                out.println("<script language='javascript'>window.location.href='staff.jsp';alert('添加入住失败！')</script>");
            }
        }
    }
