 ...
 * @author 钟镇斌
 */
@WebServlet(name = "ContinuteAddServlet", urlPatterns = {"/ContinuteAddServlet"})
public class ContinuteAddServlet extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
           ...
           String run_staff_tel=request.getParameter("run_staff_tel");
            ...
            flag=operate.insert_continute(continute_id, custom_name, custom_tel, continute_room, continute_room_type, live_begin_time, aleady_live_time, continute_begin_time, continute_end_time, continute_total_time, continute_price, run_staff_name, run_staff_tel);
             if (flag == 1) {
                out.println("<script language='javascript'>window.location.href='staff.jsp';alert('添加续房成功！')</script>");
            } else {
                out.println("<script language='javascript'>window.location.href='staff.jsp';alert('添加续房失败！')</script>");
            }
