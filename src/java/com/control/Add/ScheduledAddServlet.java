 ...
 * @author 钟镇斌
 */
@WebServlet(name = "ScheduledServlet", urlPatterns = {"/ScheduledServlet"})
public class ScheduledAddServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            flag = operate.insert_scheduled(scheduled_id, custom_name, custom_identify, custom_tel, scheduled_room, scheduled_room_type, scheduled_time, scheduled_begin_live, scheduled_end_leave, scheduled_total_time, scheduled_price);
            if (flag == 1) {
                out.println("<script language='javascript'>window.location.href='staff.jsp';alert('添加预定成功！')</script>");
            } else {
                out.println("<script language='javascript'>window.location.href='staff.jsp';alert('添加预定失败！')</script>");
            }
        }
    }
